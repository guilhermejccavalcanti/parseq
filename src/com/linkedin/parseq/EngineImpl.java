...
class EngineImpl implements Engine {
  ...
  EngineImpl(final Executor taskExecutor, final DelayedExecutor timerExecutor, final ILoggerFactory loggerFactory, final Map<String, Object> properties, final PlanDeactivationListener planActivityListener, final PlanCompletionListener planCompletionListener) {
  ...
  @Override public void run(final Task<?> task) {
    run(task, task.getClass().getName());
  }

  @Override public void run(final Task<?> task, final String planClass) {
    ...
    PlanContext planContext = new PlanContext(this, _taskExecutor, _timerExecutor, _loggerFactory, _allLogger, _rootLogger, planClass, task, _maxRelationshipsPerTrace, _planDeactivationListener, _planCompletionListener);
  ...
  @Override public boolean awaitTermination(final int time, final TimeUnit unit) throws InterruptedException {
