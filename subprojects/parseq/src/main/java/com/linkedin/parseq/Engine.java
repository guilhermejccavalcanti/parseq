...
public class Engine {
  ...
  private TaskQueueFactory createTaskQueueFactory(final Map<String, Object> properties, final TaskQueueFactory taskQueueFactory) {
    if (taskQueueFactory == null) {
      if (_properties.containsKey(DEFAULT_TASK_QUEUE)) {
        ...
        try {
          final Class<? extends TaskQueue> clazz =
  ...
  */
   ...
   * allowed to finish. Use {@link #awaitTermination(int, TimeUnit)}
   ...
   * actually stopped and {@link #awaitTermination(int, TimeUnit)}
   ...
   * {@link #awaitTermination(int, TimeUnit)} to wait for
